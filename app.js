...
function countCurrency(event) {
    ...
    let preferdNote = document.querySelector('input[name="note"]:checked');
    let amount = document.getElementById('amount').value

    //checking if the user selected preferd note or not
    if(preferdNote === null){
        preferdNote = 500;  //assigning default preferdNote
    } else{
        preferdNote = +preferdNote.value;
    }
    if (amount < 100) {
